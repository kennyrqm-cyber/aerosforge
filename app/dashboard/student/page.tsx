...
import { CheckrideEnrollmentStatus, ProgressStatus, Role } from "@/generated/prisma/client";
import { SignOutButton } from "@/components/sign-out-button";
import { baselineAverage, parseFocusAreas } from "@/lib/checkride-baseline";
import { db } from "@/lib/db";
import { requireRole } from "@/lib/session";

const blockedCheckrideStatuses = new Set<CheckrideEnrollmentStatus>([
...
export default async function StudentDashboard() {
  ...
  const [progress, badgeCount, attempts, lessonCount, latestBaseline, checkrideEnrollment] = await Promise.all([
    ...
    db.checkrideEnrollment.findFirst({
      where: { userId: session.user.id },
      include: { cohort: true, payment: { select: { status: true } } },
  ...
  return <main>
    <div className="dashboardGrid">
      ...
      <section>
        ...
        <div className="grid2">
          {checkrideEnrollment ? <div className={`card ${blockedCheckrideStatuses.has(checkrideEnrollment.status) ? "metricDanger" : ""}`}>
            <span className="badge">CHECKRIDE COHORT</span>
            <h3>Founding-member delivery</h3>
            <p><strong>{checkrideEnrollment.status.replaceAll("_", " ")}</strong></p>
            <p className="muted">{checkrideEnrollment.cohort ? `${checkrideEnrollment.cohort.name} • ${checkrideEnrollment.cohort.startsAt.toLocaleString("en-US", { timeZone: "UTC" })} UTC` : "Your cohort has not been assigned yet."}</p>
            {checkrideEnrollment.nextActionAt ? <p className="finePrint">Next operational action: {checkrideEnrollment.nextActionAt.toLocaleString("en-US", { timeZone: "UTC" })} UTC</p> : null}
            {checkrideEnrollment.payment.status !== "PAID" ? <div className="notice warningBox">Delivery is paused while the payment status is reviewed.</div> : null}
          </div> : null}
