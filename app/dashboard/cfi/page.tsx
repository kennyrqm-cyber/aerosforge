...
export default async function CfiDashboard() {
  ...
  const [assignments, reviewQueue, scenarioQueue] = await Promise.all([
    db.cfiStudentAssignment.findMany({
      ...
      include: { student: { select: { id: true, name: true, email: true, checkrideBaselines: { orderBy: { createdAt: "desc" }, take: 1 } } } },
  ...
  return <main>
    <div className="dashboardGrid">
      ...
      <section>
        ...
        <div className="card tableWrap"><h3>Assigned students</h3>{assignments.length === 0 ? <p className="muted">No active assignments.</p> : <table><thead><tr><th>Student</th><th>Status</th><th>Latest self-baseline</th><th>Student-selected focus</th></tr></thead><tbody>{assignments.map((item) => {
          const baseline = item.student.checkrideBaselines[0];
          const focus = parseFocusAreas(baseline?.focusAreas);
          return <tr key={item.id}><td>{item.student.name ?? item.student.email}</td><td>{item.status}</td><td>{baseline ? `${baselineAverage(baseline.confidenceTotal).toFixed(1)} / 5 • ${baseline.createdAt.toLocaleDateString("en-US")}` : "Not submitted"}</td><td>{focus.length ? focus.map((area) => `Area ${area.key}: ${area.title}`).join(" • ") : "—"}{baseline?.reflection ? <><br/><span className="muted finePrint">Student reflection: {baseline.reflection}</span></> : null}</td></tr>;
        })}</tbody></table>}</div>
